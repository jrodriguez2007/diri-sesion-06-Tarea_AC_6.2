...
export class FoodService {
  ...
  static async ObtenerMenu(): Promise<MenuItem[]> {
    try {
  ...
  static async ActualizarStock(id: number, cantidadRestante: number): Promise<void> {
    try {
  ...
  static async CrearMenu(item: MenuItem): Promise<void> {
    try {
      // Ruta en la base de datos
      const newItemRef = ref(db, `menuItems/${item.id}`);
      await set(newItemRef, item);
    } catch (error) {
      LoggerService.error(`Error al crear un nuevo menú: ${error}`);
      throw error;
    }
  }

  static async RegistrarVenta(item: SaleItem): Promise<void> {
